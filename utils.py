...
__all__ = [
    "xavier_uniform",
    "wn_xavier",
    "calc_kernel_minimum_variance",
    "calc_cross_utterance_speaker_code",
...
def calc_kernel_minimum_variance(content_code):
    ...
    x = (
        content_code.permute(0, 1, 3, 2)
        .contiguous()
        .view([s_dim, u_dim * t_dim, f_dim])
    )
...
def calc_cross_utterance_speaker_code(log_confidence, speaker_code_sample):
    ...
    mask = torch.ones([u_dim, u_dim], device=speaker_code_sample.device) - torch.eye(
        u_dim, device=speaker_code_sample.device
    )
    speaker_code = torch.einsum(
        "suft,sugt,ui->sug", confidence, speaker_code_sample, mask
    ) / torch.einsum("suft,ui->suf", confidence, mask)
...
def large_margin_cosine_loss(feature, margin=0.05, s=4):
    ...
    return F.cross_entropy(
        torch.exp(s * (cossim - m[:, :, None, None])),
        torch.arange(s_dim, device=feature.device)[:, None, None].expand(
            s_dim, u_dim, u_dim
        ),
    )
...
def kl_normal(x, eps=1e-4):
    ...
    diff = x - mean
    sigma = (
        diff.matmul(diff.transpose(1, 2)) / x.size(2)
        + eps * torch.eye(x.size(1), dtype=x.dtype, device=x.device)[None, :, :]
    )
    kl = (
        -sigma.logdet().sum() + sigma.diagonal(dim1=1, dim2=2).sum() + mean.pow(2).sum()
    )
